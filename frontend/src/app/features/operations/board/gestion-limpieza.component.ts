...
interface CleanRoom { id: string; number: string; floor?: string | null; status: string; typeName: string; repaso: boolean; enCurso: boolean; taskId: string | null; }
      ...
      <div class="grid">
        @for (r of normalRooms(); track r.id) {
          <article class="card" [class.curso]="r.enCurso">
            <div class="num">Hab. {{ r.number }}</div><div class="ty">{{ r.typeName }}</div><div class="pi-flo">Piso {{ r.floor || '-' }}</div>
            <div class="st">{{ r.enCurso ? 'Limpieza en curso' : 'Limpieza en espera' }}</div>
            @if (r.enCurso) {
...
export class GestionLimpiezaComponent implements OnInit {
  ...
  ngOnInit(): void { this.reload(); this.loadLinen(); }
