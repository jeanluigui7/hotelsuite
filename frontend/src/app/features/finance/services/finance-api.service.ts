...
export class FinanceApiService {
  ...
  closeCash(dto: { closingAmount: number; notes?: string; denominations?: { value: number; qty: number }[]; pettyCashLeft?: number }): Observable<ApiResponse<CloseResult>> {
  ...
  /** Corrige el DESGLOSE de pagos de una venta (método+monto por línea, mismo total). */
  ...
  voidSaleLine(id: string, itemId: string, reason?: string): Observable<ApiResponse<unknown>> {
    return this.http.post<ApiResponse<unknown>>(`${this.api}/sales/${id}/void-line`, { itemId, reason });
  }
