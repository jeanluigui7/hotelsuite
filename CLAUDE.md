...
- **Fase actual:** ✅ PROYECTO COMPLETO — FASES 0 a 10 implementadas y **VALIDADO E2E contra SQL Server real**. En producción (VPS DigitalOcean, Docker).
- **Auditoría de caja (2026-08-26, commit `775fc1b`, desplegado):** Etapas 1–6 de FINANZAS→CAJAS→MOVIMIENTOS + Kardex. Estado `AJUSTADA` + tabla `CashIntervention` (huella antes/después/motivo/usuario/tipo); soft-void de movimientos (no altera totales); acción **VER** (`/cash/movement-detail`) con historial; **venta no registrada** desde Kardex→Ajuste (COBRADA/NO_COBRADA/POR_VERIFICAR) creando `Sale` marcada (`unregistered`/`verifyStatus`); tarjeta **REGULARIZACIONES** (sin doble conteo) y **DEUDAS** interactiva; **REABRIR** solo Admin. Migraciones `20260830000000_cash_intervention` + `20260831000000_sale_unregistered`. Detalle en memoria `cash-intervention-audit`.
